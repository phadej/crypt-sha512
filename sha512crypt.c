...
static const char b16t[16] =
"0123456789abcdef";

void debug64(const char *prefix, const unsigned char digest[64]) {
    printf("%s: ", prefix);
    for (int i = 0; i < 64; ++i) {
        unsigned char c = digest[i];
        printf("%c%c", b16t[(c >> 4) & 0xf], b16t[c & 0xf]);
    }
    printf("\n");
}

void debug64b(const char *prefix, const char *seq, size_t seq_len) {
    printf("%s: ", prefix);
...
sha512_crypt_r (const char *key, const char *salt, char *buffer, int buflen)
{
  ...
  debug64b("byte sequence P", p_bytes, key_len);
  ...
  debug64b("byte sequence S", s_bytes, salt_len);
...
#ifdef EXAMPLE
int main() {
    const char *key = "xyny33AP6";
    const char *salt = "$6$saltstring";

    char buffer[512];
    sha512_crypt_r(key, salt , buffer, sizeof(buffer));
    printf("this: %s\n", buffer);

    char *tmp = crypt(key, salt);
    printf("libc: %s\n", tmp);
